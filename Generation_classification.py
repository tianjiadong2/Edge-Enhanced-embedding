...
# 百分比，十分比
adj_add = [[], []]
for id in range(int(40 * 5429 / 100)):
...
print("fenleikaishi")
for bufen in range(100):

    pd.set_option('display.max_columns',5)
    pd.set_option('display.max_rows',4000)

    X_default = pd.DataFrame(np.concatenate([X_for_generate,fake_data[:int((bufen/100)*len(fake_data))]]),columns=[f'fea{i}' for i in range(1,X_train.shape[1] + 1)])
    X_default['target'] = 1
    ...
    net = Net(8, 16, 2)  # 输入节点8个，隐层节点16个，输出节点2个

    loss_func = torch.nn.CrossEntropyLoss()  # 交叉熵
    optimizer = torch.optim.Adam(net.parameters(), lr=0.01, weight_decay=5e-4)  # 优化器
    ...
    print(log.format(bufen, best_tp, best_fn, best_fp, best_tn, best_recall_scor, best_precision_scor, best_auc_scor))